...
uint8_t xd_tmp_sp[127];

#elif defined __ICC8051__
uint8_t __xdata sppage_buffer[127];
uint8_t __xdata xd_tmp_sp[127];
...
void WriteDataToSPOnePage(uint8_t u8_addr, uint8_t *pDat, uint16_t num)
{
    ...
    {
#if defined __C51__
        xd_tmp_sp[i] = Read_SPROM_BYTE((uint8_t code *)i);
#elif defined __ICC8051__
        xd_tmp_sp[i] = Read_SPROM_BYTE((uint8_t __code *)i);
#elif defined __SDCC__
        xd_tmp_sp[i] = Read_SPROM_BYTE((uint8_t __code *)i);
#endif
    }
